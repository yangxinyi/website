...
public class MD5Custom
{
    ...
    public MD5Custom(String input, String enc) {
        byte bytes[] = null;
        try {
    ...
    public MD5Custom(String input) {
        this(input, "UTF8");
    }

    public MD5Custom(InputStream in) {
        this.stringp = false;
        this.in = in;
        this.state = new int[4];
        this.buffer = new byte[64];
    ...
    public void hmac_Md5(String text, String key) throws IOException {
        ...
        if (key_len > 64) {
            key1 = new MD5Custom(key).getDigest();
            key_len = 16;
        }
        ...
        MD5Custom md51 = new MD5Custom("");
        md51.update(k_opad, 64);
        md51.update(digest, 16);
        digest = md51.getDigest();
    }
    public void hmac_Md5(byte[] text, byte[] key) throws IOException {
        byte disgest[] = new byte[16];
        byte output[] = new byte[32];
        // output[33] = ( byte ) '\0' ;
        byte k_ipad[] = new byte[64];
    ...
    public static String encrypt(String str) {
        try {
            MD5Custom md5 = new MD5Custom("");
            md5.hmac_Md5(str, makeDisruptCode());
    ...
    public static String encrypt(File file) throws IOException{
        ...
        while((readNum = fr.read(buffer)) > -1){
        ...
        try {
            MD5Custom md5 = new MD5Custom("");
            md5.hmac_Md5(bs, makeDisruptCode().getBytes());
            byte[] b = md5.getDigest();
            return stringify(b);
        } catch (Exception localException) {
            localException.printStackTrace();
    ...
    public static String encrypt(byte[] str) {
        try {
            MD5Custom md5 = new MD5Custom("");
            md5.hmac_Md5(str, makeDisruptCode().getBytes());
            byte[] b = md5.getDigest();
            return stringify(b);
        } catch (Exception localException) {
            localException.printStackTrace();
    ...
    private static String makeDisruptCode(){
        return "hellokitty";
    } 
